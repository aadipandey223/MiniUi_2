...
// Room Sidebar Management
function showRoomSidebar(roomData) {
    ...
    if (roomNameEl) roomNameEl.textContent = roomData.name || 'Private Room';
    if (roomCodeEl) roomCodeEl.textContent = roomData.id || currentRoomId;
    if (onlineCountEl) onlineCountEl.textContent = roomData.userCount || '1';

    // Show sidebar
    sidebar.classList.remove('hidden');
}

function hideRoomSidebar() {
    const sidebar = document.getElementById('roomSidebar');
    if (sidebar) {
...
async function joinRoom(roomId, password = null) {
    ...
    showRoomSidebar({
        id: roomId,
        name: roomId.substring(0, 8).toUpperCase(),
        userCount: 1
    });
}
...
const uploadHistory = new Array(30).fill(0);
const downloadHistory = new Array(30).fill(0);
...
function initSpeedGraph() {
    ...
    // Set canvas size
    speedGraph.width = speedGraph.offsetWidth || 280;
...
function updateSpeedGraph() {
    const uploadMB = performanceMetrics.currentUploadSpeed / (1024 * 1024);
    const downloadMB = performanceMetrics.currentDownloadSpeed / (1024 * 1024);

    uploadHistory.push(uploadMB);
    downloadHistory.push(downloadMB);

    if (uploadHistory.length > maxSpeedDataPoints) uploadHistory.shift();
    if (downloadHistory.length > maxSpeedDataPoints) downloadHistory.shift();

    drawSpeedGraph();
}

// Draw premium speed graph
function drawSpeedGraph() {
    ...
    const maxVal = Math.max(...uploadHistory, ...downloadHistory, 0.5);

    function drawLine(data, color, fillOpacity) {
        ctx.beginPath();
        ...
        data.forEach((val, i) => {
            const x = (i / (maxSpeedDataPoints - 1)) * w;
    ...
    drawLine(downloadHistory, '#8b5cf6', 0.1);
    drawLine(uploadHistory, '#06b6d4', 0.1);
}
...
function updateStatsDisplay() {
    // Current Speeds
    const uploadEl = document.getElementById('currentUploadSpeed');
    const downloadEl = document.getElementById('currentDownloadSpeed');
    if (uploadEl) uploadEl.textContent = formatSpeed(performanceMetrics.currentUploadSpeed);
    if (downloadEl) downloadEl.textContent = formatSpeed(performanceMetrics.currentDownloadSpeed);

    // Grid counters
    const connectionsEl = document.getElementById('activeConnections');
    const queuedEl = document.getElementById('queuedCount');
    if (connectionsEl) connectionsEl.textContent = semaphore.currentCount;
    if (queuedEl) queuedEl.textContent = downloadQueue.queue.filter(q => q.status === 'waiting').length;

    // Resource Utilization
    const utilValue = document.getElementById('utilizationValue');
    const utilBar = document.getElementById('utilizationBar');
    if (utilValue && utilBar) {
        const utilPercent = Math.round((semaphore.currentCount / semaphore.maxConcurrent) * 100);
        utilValue.textContent = `${utilPercent}%`;
        utilBar.style.width = `${utilPercent}%`;
    }

    // Performance Metrics
    const avgWaitEl = document.getElementById('avgWaitTime');
    const throughputEl = document.getElementById('throughputValue');
    if (avgWaitEl) avgWaitEl.textContent = `${performanceMetrics.averageWaitTime.toFixed(1)}s`;
    if (throughputEl) throughputEl.textContent = `${performanceMetrics.totalDownloads}/min`;

    const peakEl = document.getElementById('peakSpeedValue');
    if (peakEl) peakEl.textContent = `${(performanceMetrics.peakDownloadSpeed / (1024 * 1024)).toFixed(2)} MB/s`;

    // Semaphore Banner
    const semCount = document.getElementById('semaphoreCount');
    const semDots = document.getElementById('semaphoreDots');
    if (semCount) semCount.textContent = `${semaphore.currentCount}/${semaphore.maxConcurrent}`;
    if (semDots) {
        let dotsHTML = '';
        for (let i = 0; i < semaphore.maxConcurrent; i++) {
            dotsHTML += `<span class="dot ${i < semaphore.currentCount ? 'filled' : ''}"></span>`;
        }
        semDots.innerHTML = dotsHTML;
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const sidebarCreateBtn = document.getElementById('createRoomBtn');
    const sidebarJoinBtn = document.getElementById('joinRoomBtn');
    if (sidebarCreateBtn) sidebarCreateBtn.onclick = showCreateRoomModal;
    if (sidebarJoinBtn) sidebarJoinBtn.onclick = showJoinRoomModal;

    // Incognito Mode Toggle
    if (incognitoToggle) {
        incognitoToggle.onchange = () => {
            document.body.classList.toggle('incognito-mode', incognitoToggle.checked);
            showToast(`Incognito Mode ${incognitoToggle.checked ? 'Enabled' : 'Disabled'}`, 'info');
        };
    }
...
function renderActiveDownloads() {
    ...
    // Updates are handled in renderTransferQueues for the list items, 
    // but we can update active counts here
    const activeCountBadge = document.getElementById('activeCountBadge');
    if (activeCountBadge) {
        activeCountBadge.textContent = `${downloads.length} active`;
    }
}
...
// Redundant graph logic removed - handled by sidebar speedGraph
...
// Helper to get iconography based on file extension
function getFileIcon(fileName) {
    const ext = fileName.split('.').pop().toLowerCase();

    // 3D-style SVG Icons matching mockup
    const icons = {
        zip: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(139, 92, 246, 0.15)"/><path d="M16 14h16v20H16V14z" fill="rgba(139, 92, 246, 0.4)"/><path d="M24 18v12M20 22h8" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`,
        rar: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(139, 92, 246, 0.15)"/><path d="M16 14h16v20H16V14z" fill="rgba(139, 92, 246, 0.4)"/><path d="M24 18v12M20 22h8" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`,
        pdf: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(239, 68, 68, 0.15)"/><path d="M16 14h16v20H16V14z" fill="rgba(239, 68, 68, 0.4)"/><path d="M20 20h8v8h-8z" stroke="currentColor" stroke-width="2"/></svg>`,
        mp4: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(59, 130, 246, 0.15)"/><path d="M14 18l20 0l-10 12z" fill="rgba(59, 130, 246, 0.4)" stroke="currentColor" stroke-width="2"/></svg>`,
        mov: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(59, 130, 246, 0.15)"/><path d="M14 18l20 0l-10 12z" fill="rgba(59, 130, 246, 0.4)" stroke="currentColor" stroke-width="2"/></svg>`,
        fig: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(16, 185, 129, 0.15)"/><circle cx="24" cy="24" r="10" fill="rgba(16, 185, 129, 0.4)" stroke="currentColor" stroke-width="2"/></svg>`,
        default: `<svg viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="12" fill="rgba(148, 163, 184, 0.15)"/><path d="M16 12h16v24H16V12z" stroke="currentColor" stroke-width="2"/></svg>`
    };

    return icons[ext] || icons.default;
}

// Render available files in premium card format
function renderAvailableFilesCompact() {
    const listEl = document.getElementById('availableFiles');
    if (!listEl) return;
    ...
    if (availableFiles.length === 0) {
        listEl.innerHTML = `
            <div class="empty-state-container" style="grid-column: 1 / -1;">
                ...
                <p class="empty-state-text">No available files</p>
                <p class="empty-state-subtext">Waiting for peers to share...</p>
            </div>
        `;
        return;
    }
    ...
    listEl.innerHTML = availableFiles.map(file => {
        ...
        // Mocking some metadata that would usually come from server/crypto-client
        const hasExpiry = Math.random() > 0.5; // Visual demo
        const isEncrypted = true;
        const downloadCount = Math.floor(Math.random() * 10);
        const timeLeft = "59m 44s"; // Visual demo

        return `
            <div class="file-card ${isSelected ? 'selected' : ''}" onclick="toggleFileSelection('${file.id}')">
                ...
                <div class="card-details">
                    <div class="detail-row expiry-time">
                    ...
                    ${isEncrypted ? `
                    <div class="detail-row status-encrypted">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                            <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
                        </svg>
                        <span>Encrypted</span>
                    </div>` : ''}
                    <div class="detail-row download-stats">
                        <span>${downloadCount} download${downloadCount !== 1 ? 's' : ''}</span>
                    </div>
                </div>
                <div class="card-buttons">
                    <button class="btn-card-download" onclick="event.stopPropagation(); window.downloadFile('${file.id}')">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <button class="btn-card-delete" onclick="event.stopPropagation(); window.deleteAvailableFile('${file.id}')">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
// Render transfer queue with premium list items
function renderTransferQueues() {
    const listEl = document.getElementById('queueList');
    if (!listEl) return;

    const items = [...downloadQueue.queue];

    // Update badge counts
    const activeCount = items.filter(i => i.status === 'running').length;
    const queuedCount = items.filter(i => i.status === 'waiting').length;

    const activeBadge = document.getElementById('activeCountBadge');
    const queuedBadge = document.getElementById('queuedCountBadge');

    if (activeBadge) activeBadge.textContent = `${activeCount} active`;
    if (queuedBadge) queuedBadge.textContent = `${queuedCount} queued`;

    if (items.length === 0) {
        listEl.innerHTML = `
            <div class="empty-state-container">
                <p class="empty-state-text">No active transfers</p>
            </div>
        `;
        return;
    }
    ...
    listEl.innerHTML = items.map(item => {
        const transfer = Array.from(activeTransfers.values()).find(t => t.fileId === item.fileInfo.id);
        const progress = transfer ? transfer.progress : 0;
        const speed = transfer ? formatSpeed(transfer.speed) : 'Waiting...';
        const isRunning = item.status === 'running';

        return `
            <div class="queue-item ${isRunning ? 'active' : 'waiting'}">
                ...
                <div class="queue-icon-box">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" ${isRunning ? 'style="transform: rotate(180deg)"' : ''}>
                        <path d="M7 13l5 5 5-5M12 18V6"/>
                    </svg>
                </div>
                <div class="item-main">
                    <div class="item-top">
                        <span class="item-name">${escapeHtml(item.fileInfo.name)} ${isRunning ? '<svg class="icon-bolt" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>' : ''}</span>
                    </div>
                    <div class="item-meta">
                ...
                <div class="item-actions">
                    ...
                    <button class="action-btn remove" onclick="window.removeFromQueue(${item.id})">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
